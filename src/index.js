    ...
    try {
        ...
        if (decodedEmailToken) {
            ...
            userLogin.findOne({ username: req.body.username }).then(async (docs) => {
                ...
                console.log("docs", docs.emailToken)
                if (docs.emailToken !== "") {
                const decodedEmailToken1 = jwt.verify(docs.emailToken, JWT_SECRET)
                //token valid ? expired or what
                //if all ok encrypt body password and update in db
                if (req.body.emailToken == docs.emailToken) {
                    if (decodedEmailToken.name == decodedEmailToken1.name) {
                        var salt = await bcrypt.genSalt(10)
                        var hash = await bcrypt.hash(req.body.password, salt)
                        req.body.password = hash
                        userLogin.updateOne({ _id: docs._id },
                    ...
                    res.send({ message: "TOKEN MISMATCH ! Not Authorized Try again using Forgot password !" });
                }

                } else {
                    res.send({ message: "Not Authorized Try again using Forgot password !" });
                }
            }
            )
        }
